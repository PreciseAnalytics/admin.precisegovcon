...
export async function GET(req: NextRequest) {
  ...
  for (const record of all) {
    const existing = await (prisma as any).contractors.findUnique({
      where: { uei_number: record.uei_number },
    });
    if (!existing) newCount++;

    await (prisma as any).contractors.upsert({
      where:  { uei_number: record.uei_number },
      create: {
        ...record,
        contacted: false, enrolled: false,
        contact_attempts: 0, priority: 'Medium',
        score: Math.floor(Math.random() * 40) + 55,
        created_at: new Date(), synced_at: new Date(),
      },
      update: {
        name: record.name, email: record.email,
        sam_gov_id: record.sam_gov_id, cage_code: record.cage_code,
        naics_code: record.naics_code, state: record.state,
